...
def notify(message):
    ...
    if telegram_gm_id:
        __notify(telegram_bot_token, telegram_gm_id, telegram_gm_prefix + message)
...
def create_day_file(client, symbol, interval, day_dt):
    ...
    end_dt = start_dt + timedelta(hours=23, minutes=59, seconds=59, microseconds=999999)
...
def get_expected_num_of_lines_in_day_file(day_dt, interval_as_timedelta, utc_now=None):
...
def get_klines_from_day_files(
    client, symbol, interval, start_ts=None, end_ts=None, limit=500
):
    """
...
start_ts=sts, end_ts=ets)
    ...
    if start_ts:
        ...
        logger.debug(
            "Start time snapped by %s: %s -> %s", interval, start_dt_orig, start_dt
        )
    else:
    ...
    for day_dt in day_dts:
        ...
        if day_dt == today:
            ...
            if len(lines) < expected_num_of_lines:  # Fetch from API again.
                ...
                logger.debug(
                    f"Day file %s has %s records " f"which are below expected %s",
                    file_path,
                    len(lines),
...
class VWMASignalGenerator(SignalGenerator):
    ...
    def get_signal(self, dt, symbol, interval):
        ...
        ohlc4 = Series(
            (x["close"] + x["open"] + x["high"] + x["low"]) / 4 for x in klines
        )

        vwma_arr = vwma(ohlc4, Series(x["volume"] for x in klines), self.length)
        ...
        return ActionType.STAY, klines, f"No new signal generated"
...
class SMASignalGenerator(SignalGenerator):
    ...
    def get_signal(self, dt, symbol, interval):
        ...
        if (
            df.tail(1).sma.item() <= df.tail(1).close.item() <= max_buy_price
            and buy_price == 0
        ):
            return (
                ActionType.BUY,
                ...
                % (
                    decimal_as_str(df.tail(1).close.item()),
                    decimal_as_str(max_buy_price),
                ),
            )

        if (
            df.tail(1).close.item() <= min_sell_price
            and df.tail(1).close.item() < df.tail(1).open.item()
        ):
            return (
                ActionType.SELL,
                klines,
...
class ConsecutiveUpDownSignalGenerator(SignalGenerator):
    ...
    def get_signal(self, dt, symbol, interval):
        needed_num_of_candles = (
            max([self.num_of_reds_to_sell, self.num_of_greens_to_buy]) + 1
        )
        interval_as_timedelta = interval_to_timedelta(interval)
        end_dt = floor_dt(dt, interval_as_timedelta)
        start_dt = end_dt - interval_as_timedelta * needed_num_of_candles
        ...
        if len(klines) < needed_num_of_candles:
            return (
                ActionType.ERR,
                klines,
                f"Requested {needed_num_of_candles} klines, " f"{len(klines)} returned",
            )
...
class AllInActionGenerator(ActionGenerator):
    ...
    def __init__(self, client, signal_generators=None, investment_multiplier=1):
    ...
    def get_action(self, dt, symbol, interval):
        ...
        for name, generator in self.signal_generators.items():
            ...
            logger.debug(
                "%s returned %s on %s. Reason: %s", name, signal.value, dt, reason
            )
        ...
        if most_common_signal == ActionType.SELL:
            ...
            if base_asset_quantity <= market_min_quantity:
                return (
                    ...
                    + decimal_as_str(base_asset_quantity)
                    + ") (MARKET_LOT_SIZE)",
                )
        ...
        if most_common_signal == ActionType.BUY:
            if not self.positions:
                tradable_asset_quantity = (
                    Decimal(self.client.get_asset_balance(asset=quote_asset)["free"])
...
quantity=1, type=ORDER_TYPE_MARKET)
        ...
        """
        if (
            "newOrderRespType" in params
            and params["newOrderRespType"] is not ORDER_RESP_TYPE_RESULT
        ):
            raise BinanceAPIException(
                FakeResponse(
                    400,
        ...
        if params["type"] is not ORDER_TYPE_MARKET:
            raise BinanceAPIException(
                FakeResponse(
                    ...
                    {
                        "code": 666,
                        "msg": f"Fake client only accepts MARKET as" f"order type",
                    },
                )
            )
        """
        ...
        if quantity:
            ...
            logger.debug(
                "Calculated quote order quantity: %s",
        ...
        try:
            ...
            ratio = 0
        logger.info(
            "On %s Total Worth: %s USDT Success Ratio: %s",
            now_as_utc,
            decimal_as_str(new_worth),
            ratio_as_str(ratio),
        )
        return result
    ...
    def generate_order_chart(self, symbol, dt, interval, base_asset):
        ...
        mpf.plot(
            ...
            addplot=mpf.make_addplot(
                df["marker"], scatter=True, color="blue", marker="o", markersize=50
            ),
        )
...
def trade(base_asset, trade_assets, interval, investment_ratio):
    ...
    action_generator = AllInActionGenerator(
        client,
        signal_generators={"VWMA": VWMASignalGenerator(client)},
        investment_multiplier=investment_ratio,
    )
    ...
    if message:
        ...
        message += f"\n🤑 *Total Worth*:\n"
        message += f"• {decimal_as_str(assets_as_usdt)} USDT " f"(TWT not Included)"
        notify(message)
